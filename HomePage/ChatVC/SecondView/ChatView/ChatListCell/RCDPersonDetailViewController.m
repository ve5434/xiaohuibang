...
//    [self setLayoutForSelf];
  ...
  if (portraitUri.length == 0) {
    ...
    [self.ivAva sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",portraitUri]]];
  }
...
- (IBAction)btnConversation:(id)sender {
  ...
  chatViewController.targetId = self.userId;
    ...
    title.text = self.lblName.text;
    title.textAlignment = NSTextAlignmentCenter;
    title.textColor = [UIColor whiteColor];
    chatViewController.navigationItem.titleView = title;
